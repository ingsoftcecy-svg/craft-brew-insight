...
export function AppHeader() {
  ...
  const tc =
    turno && TURNO_CONFIG[turno]
      ? TURNO_CONFIG[turno]
      : { label: turno ?? "Desconocido", bg: "bg-gray-100", text: "text-gray-600" };
  ...
  return (
    <header className="sticky top-0 z-20 flex h-14 items-center gap-4 border-b border-slate-200/60 bg-white/60 backdrop-blur-lg px-5 shadow-sm transition-all duration-300 print:hidden">
      ...
      <div className="hidden md:flex flex-col leading-tight">
        ...
        <span className="text-[11px] text-slate-500 font-medium">{format(now, "HH:mm")} hrs</span>
      </div>

      {/* Turno badge */}
      <div className="hidden lg:flex items-center ml-2">
        <span
          className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-bold shadow-sm border border-slate-200/50 ${tc.bg} ${tc.text}`}
        >
          {tc.label}
        </span>
      </div>
      ...
      <div className="ml-auto flex items-center gap-1">
        <Button
          variant="ghost"
          size="icon"
          className="h-8 w-8 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100/80 transition-colors"
        >
